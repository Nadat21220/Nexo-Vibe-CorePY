...
export default function SociosPage() {
  ...
  const handleSave = async () => {
    ...
    } catch (e) { console.error("Error al guardar socio:", e); }
  };
  ...
  const clientesVIP = filteredClients.filter(c => c.rango === 'vip');
  const clientesNormales = filteredClients.filter(c => c.rango !== 'vip');
    ...
    <div className="flex-1 flex flex-col bg-background overflow-hidden relative">
      ...
      <main className="p-8 max-w-7xl mx-auto w-full flex-1 overflow-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-surface-200 border border-surface-400 rounded-2xl p-8 min-h-[500px] shadow-lg flex flex-col">
            ...
            <div className="space-y-6 flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {loading ? <p className="text-surface-600 text-sm">Cargando...</p> : filteredEmployees.map(e => (
                <div key={e.id} className="flex items-center justify-between group p-2 rounded-xl hover:bg-surface-300/30 transition-all border border-transparent hover:border-surface-400">
          ...
          {/* COLUMNA 2: CLIENTES IMPORTANTES */}
          <div className="bg-surface-200 border border-surface-400 rounded-2xl p-8 min-h-[500px] shadow-lg flex flex-col">
            <div className="flex items-center mb-6">
              <BarChart className="w-4 h-4 text-primary mr-2" />
              <h3 className="text-sm font-bold uppercase tracking-widest text-foreground">CLIENTES VIP</h3>
            </div>

            <div className="space-y-6 flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {loading ? <p className="text-surface-600 text-sm">Cargando...</p> : clientesVIP.length === 0 ? (
                <p className="text-surface-600 text-[10px] uppercase tracking-widest text-center mt-10 italic">No hay clientes VIP</p>
              ) : clientesVIP.map(c => (
                <div key={c.id} className="flex items-center justify-between group p-2 rounded-xl hover:bg-surface-300/30 transition-all border border-transparent hover:border-surface-400">
                  <div className="flex items-center flex-1 cursor-pointer" onClick={() => handleEdit(c, 'cliente')}>
                    <div className="w-10 h-10 rounded-full bg-surface-500 mr-4 overflow-hidden border border-primary/30">
                      <img src={`https://i.pravatar.cc/150?u=${c.email}`} alt={c.nombre} className="w-full h-full object-cover" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="font-bold text-white group-hover:text-primary transition-colors truncate">{c.nombre} {c.apellido}</h4>
