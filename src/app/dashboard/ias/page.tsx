...
const CATEGORIES = [
  { id: 'diseno_web', name: 'Diseño Web', icon: Code, color: '#00C48C' },
  { id: 'creador_ugc', name: 'Creador UGC', icon: Video, color: '#FF3B30' },
  { id: 'marketing', name: 'Marketing', icon: Megaphone, color: '#FFB800' }
];
...
export default function IASPage() {
  ...
  const fetchTasks = async (currentRole?: string | null, currentEmail?: string) => {
    ...
    try {
      ...
      if (r === 'empleado' && e) {
        // Force store init + merge local tasks with API
        fetchedTasks = mergeWithApi(data.tareas || [], e);
        console.log('[IAS] Employee tasks after merge:', fetchedTasks.length);
      }
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    if (task.estado === 'done') return false;
