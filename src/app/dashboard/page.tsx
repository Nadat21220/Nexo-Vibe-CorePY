...
import { mergeWithApiClient, getClientSubscription, upgradeClientSubscription } from '@/lib/clienteStore';
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      if (role === 'empleado' && user?.email) {
        // Use the shared store — includes the 2 mocks + any project created in Kanban
        fetchedTasks = mergeWithApi(data.allTasks || [], user.email);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        currentUrgentTasks = fetchedTasks.filter((t: any) => t.prioridad === 'inmediata' || t.prioridad === 'high');
      } else if (role === 'cliente' && user?.email) {
        fetchedTasks = mergeWithApiClient(data.allTasks || [], user.email);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        currentUrgentTasks = fetchedTasks.filter((t: any) => t.prioridad === 'inmediata' || t.prioridad === 'high');
      }
      ...
      if (role === 'empleado' || role === 'cliente') {
        ...
        fetchedTasks.forEach((t: any) => {
          if (t.estado === 'done') calcCompletadas++;
          else if (t.estado === 'in_progress') calcEnProceso++;
          else if (t.estado === 'review') calcRevision++;
          else calcPendientes++;
